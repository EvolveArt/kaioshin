...
macro_rules! autonomous_mock_runtime {
    ($mock_runtime:ident, $disable_transaction_fee:expr, $disable_nonce_validation: expr) => {
		pub mod $mock_runtime {
			...
			impl pallet_autonomous::Config for MockRuntime {
				type MaxGas = ConstU64<1_000_000>;
				type ValidityMaxOffset = ConstU64<1_000>;
			}
...
autonomous_mock_runtime!(default_mock, false, false);
autonomous_mock_runtime!(fees_disabled_mock, true, false);
autonomous_mock_runtime!(no_nonce_validation_mock, true, true);
